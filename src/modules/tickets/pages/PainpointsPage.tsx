            ...
            <div className={`bg-white border-b border-[#e2e8f0] px-6 h-14 flex items-center justify-between shadow-sm sticky top-0 z-30 font-sans transition-transform duration-500 ease-in-out ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden lg:block">
                        <h2 className="text-[13px] font-semibold text-[#002855] uppercase tracking-[0.15em] leading-none">Puntos Críticos</h2>
                        <div className="flex items-center gap-2 text-[9px] font-semibold text-[#64748b] uppercase tracking-widest mt-1">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button className="px-5 py-2 bg-amber-600 text-white font-semibold text-[10px] uppercase tracking-widest hover:bg-amber-700 transition-all border border-amber-500 shadow-md flex items-center gap-2 group">
                        ...
                        {painpoints.map(pp => (
                            <div key={pp.id} className="bg-white border border-slate-200 shadow-sm hover:shadow-xl hover:border-amber-200 transition-all duration-300 relative group overflow-hidden">
                                ...
                                <div className="p-6">
                                    ...
                                    <h4 className="text-[15px] font-semibold text-[#002855] uppercase tracking-tight mb-4 leading-tight group-hover:text-amber-600 transition-colors">{pp.title}</h4>
                                    
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between text-[11px] border-b border-slate-100 pb-2">
                                            <span className="font-semibold text-slate-400 uppercase tracking-widest">Ubicación Afectada</span>
                                            <span className="font-semibold text-[#002855] uppercase">{pp.location}</span>
                                        </div>
                                        <div className="flex items-center justify-between text-[11px] border-b border-slate-100 pb-2">
                                    ...
                                    <button className="mt-8 w-full py-4 border border-slate-100 bg-slate-50 text-[10px] font-semibold text-[#002855] uppercase tracking-[0.2em] hover:bg-[#002855] hover:text-white transition-all flex items-center justify-center gap-2 group/btn">
                        ...
                        <Table>
                            <TableHeader>
                                <tr>
                                    ...
                                    <TableHead>Incidencia / Cat.</TableHead>
                            ...
                            <TableBody>
                                {painpoints.map(pp => (
                                    <TableRow key={pp.id} className="cursor-pointer">
                                        <TableCell><span className="text-[11px] font-mono font-semibold text-slate-300 uppercase tracking-widest">{pp.id}</span></TableCell>
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-semibold text-[#002855] uppercase tracking-tight group-hover/row:text-amber-600 transition-colors">{pp.title}</span>
                                                <span className="text-[9px] font-semibold text-amber-500 uppercase tracking-widest mt-1 italic">{pp.category}</span>
                                            </div>
                                        </TableCell>
