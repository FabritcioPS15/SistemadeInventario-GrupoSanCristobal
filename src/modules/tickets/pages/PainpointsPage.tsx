            ...
            <div className="p-8 space-y-8 flex-1 overflow-y-auto max-w-[1600px] mx-auto w-full">
                <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-[13px] font-black text-[#002855] leading-tight tracking-[0.2em]">Registro de Incidencias</h3>
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between text-[11px] border-b border-slate-100 pb-2">
                                            <span className="font-bold text-slate-400 uppercase tracking-widest">Ubicación Afectada</span>
                            ...
                            <TableBody>
                                {painpoints.map(pp => (
                                    <TableRow key={pp.id} className="cursor-pointer">
                                        <TableCell><span className="text-[11px] font-mono font-bold text-slate-300 uppercase tracking-widest">{pp.id}</span></TableCell>
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-black text-[#002855] uppercase tracking-tight group-hover/row:text-amber-600 transition-colors">{pp.title}</span>
                                                <span className="text-[9px] font-bold text-amber-500 uppercase tracking-widest mt-1 italic">{pp.category}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell><span className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{pp.location}</span></TableCell>
                                        <TableCell className="text-center">
                                            <span className={`px-4 py-1.5 text-[10px] font-black tracking-wider border ${pp.status === 'Crítico' ? 'border-rose-200 text-rose-600 bg-rose-50/50' : 'border-amber-200 text-amber-600 bg-amber-50/50'}`}>{pp.status}</span>
                                        </TableCell>
