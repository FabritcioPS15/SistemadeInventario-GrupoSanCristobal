...
export default function Breadcrumbs() {
    ...
    return (
        <nav className="flex px-8 py-3 bg-white/50 backdrop-blur-sm border-b border-slate-100 items-center gap-2 overflow-x-auto no-scrollbar shrink-0">
            ...
            >
                <Home size={14} />
                            <span className="text-[10px] font-normal tracking-widest hidden sm:block">Inicio</span>
            </Link>
            ...
            {pathnames.map((value, index) => {
                ...
                return (
                    <div key={to} className="flex items-center gap-2 shrink-0">
                        {last ? (
                            <span className="text-[10px] font-normal text-blue-600 tracking-widest bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100">
                                {label}
                            </span>
                        ) : (
                            <Link
                                to={to}
                                className="text-[10px] font-normal text-slate-400 hover:text-blue-600 tracking-widest transition-colors"
                            >
                                {label}
                            </Link>
                        )}
